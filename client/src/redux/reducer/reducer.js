...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case FILTER_TEMPERAMENTS: {
      ...
      const filteredDogs = state.FilteredDogs.filter((dog) => {
        return dog.temperamento && dog.temperamento.includes(selectedTemperament);
      });
    ...
    case ORDER_WEIGHT: {
      ...
      if (order === 'minWeight') {
        sortedDogsWeight = [...state.FilteredDogs].sort((dog1, dog2) => {
          ...
          const weightA = parseInt(dog1.peso.split(' - ')[0]);
          const weightB = parseInt(dog2.peso.split(' - ')[0]);
        ...
        sortedDogsWeight = [...state.FilteredDogs].sort((dog1, dog2) => {
          ...
          const weightA = parseInt(dog1.peso.split(' - ')[1]);
          const weightB = parseInt(dog2.peso.split(' - ')[1]);
